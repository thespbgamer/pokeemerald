...
u8 TypeEffectiveness(u8 targetId)
{
    ...
    moveFlags = TypeCalcWithHiddenPower(move, gBattleMons[targetId].species, gBattleMons[targetId].ability);
...
static void MoveSelectionDisplayMoveType(void)
{
    ...
    *(txtPtr)++ = FONT_NORMAL;
    ...
    if (moveInfo->moves[gMoveSelectionCursor[gActiveBattler]] == MOVE_HIDDEN_POWER)
    {
        ...
        typeColor = IsDoubleBattle() ? B_WIN_MOVE_TYPE : TypeEffectiveness(GetBattlerAtPosition(BATTLE_OPPOSITE(GetBattlerPosition(gActiveBattler))));
    }
    else
    {
        StringCopy(txtPtr, gTypeNames[gBattleMoves[moveInfo->moves[gMoveSelectionCursor[gActiveBattler]]].type]);
